...
export async function createUser({ email, passwordHash, name, memberId, role }) {
  ...
  await (await usersColl()).insertOne({ _id, email: _id, passwordHash, name: name || "", memberId: memberId || null, role: role === "admin" ? "admin" : "member", createdAt: Date.now() });
  return _id;
}
...
export async function setUserRole(email, role) {
  await (await usersColl()).updateOne({ _id: email.toLowerCase() }, { $set: { role: role === "admin" ? "admin" : "member" } });
}
...
export async function createSession({ email, name, memberId, role }) {
  ...
  await (await sessionsColl()).insertOne({ _id: token, email, name: name || "", memberId: memberId || null, role: role === "admin" ? "admin" : "member", createdAt: new Date(), expiresAt });
  return token;
}
